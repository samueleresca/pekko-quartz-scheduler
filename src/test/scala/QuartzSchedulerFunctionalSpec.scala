...
class QuartzSchedulerFunctionalSpec(_system: ActorSystem)
  ...
  "The Quartz Scheduling Extension" must {
    ...
    "Properly Setup & Execute a Cron Job with correct fireTime" in {
      ...
      val probe = TestProbe()
      receiver ! NewProbe(probe.ref)
      val jobDt = QuartzSchedulerExtension(_system).schedule(
        "cronEvery10SecondsWithFireTime",
        receiver,
        MessageRequireFireTime(Tick)
      )
    ...
    "Delayed Setup & Execute a Cron Job" in {
      ...
      /* This is a somewhat questionable test as the timing between components may not match the tick off. */
      val receipt = probe.receiveWhile(Duration(30, SECONDS), Duration(30, SECONDS), 2) { case Tock =>
        Tock
      }
  ...
  "Get next trigger date by schedule name" in {
    ...
    receiver ! NewProbe(probe.ref)
    val jobDt =
      QuartzSchedulerExtension(_system).schedule("cronEveryMidnight", _system.actorSelection(receiver.path), Tick)
  ...
  "The Quartz Scheduling Extension with Dynamic Create" must {
    ...
    "Add new, schedulable schedule with valid inputs" in {
      ...
      QuartzSchedulerExtension(_system).createSchedule(
        "nonExistingCron",
  ...
  "The Quartz Scheduling Extension with Dynamic create, update, delete JobSchedule operations" must {
    ...
    "Reschedule an existing job schedule Cron Job" in {
      ...
      noException should be thrownBy {
        val newFirstTimeTriggerDate = QuartzSchedulerExtension(_system).updateJobSchedule(
          toRescheduleJobName,
          receiver,
          Tick,
    ...
    "Delete an existing job schedule Cron Job without any error and allow successful creation of new schedule with identical job name" in {
      ...
      noException should be thrownBy {
        ...
        if (success) {
          ...
          val newJobDt = QuartzSchedulerExtension(_system).createJobSchedule(
            toDeleteSheduleJobName,
            receiver,
            Tick,
  ...
  /**
   * Mass operations {suspendAll, resumeAll, deleteAll} combine existing QuartzSchedulerExtension {pauseAll, resumeAll}
   * and adds deleteAll (suspend synonym for pause for consistency).
   */
  "The Quartz Scheduling Extension with Dynamic mass methods" should {
    ...
    "Delete all jobs" in {
      ...
      noException should be thrownBy {
        ...
        val newJobDt1 = QuartzSchedulerExtension(_system).createJobSchedule(
          toBeDeletedAllJobName1,
          receiver,
          Tick,
          Some("Creating new dynamic schedule after deleteAll success"),
          "8 * * ? * *"
        )
